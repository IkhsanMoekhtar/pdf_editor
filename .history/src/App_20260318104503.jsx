...
function App() {
  ...
  // --- 3. STATE ACTION BAR & INTERAKSI ---
  const [userZoom, setUserZoom] = useState(1); 
  const [rotation, setRotation] = useState(0); 
  const [flipX, setFlipX] = useState(false); 
  const [flipY, setFlipY] = useState(false); 
  const [isZoomMode, setIsZoomMode] = useState(false); // Mode Scroll-to-Zoom

  // --- 4. STATE DRAG / PANNING ---
  const [isDragging, setIsDragging] = useState(false);
  const [dragStart, setDragStart] = useState({ x: 0, y: 0 });
  const [scrollStart, setScrollStart] = useState({ left: 0, top: 0 });

  // --- EFEK A: Mengukur area layar (ResizeObserver) ---
  useEffect(() => {
    const observer = new ResizeObserver((entries) => {
      if (entries[0]) {
        setContainerSize({
          width: entries[0].contentRect.width - 60, 
          height: entries[0].contentRect.height - 130 
        });
      }
    });
  ...
  // --- EFEK B: Menangkap Scroll Mouse untuk Zoom ---
  useEffect(() => {
    const container = scrollContainerRef.current;
    if (!container) return;

    const handleWheel = (e) => {
      if (!isZoomMode) return; // Jika mode zoom mati, biarkan scroll normal
      
      e.preventDefault(); // Blokir scroll web bawaan
      const zoomStep = 0.1; 
      
      if (e.deltaY < 0) {
        setUserZoom((prev) => Math.min(prev + zoomStep, 4)); // Zoom In
      } else {
        setUserZoom((prev) => Math.max(prev - zoomStep, 0.25)); // Zoom Out
      }
    };
  ...
  // --- FUNGSI KALKULASI ZOOM OTOMATIS (FIT-TO-BOX) ---
  const calculateBaseScale = () => {
    // Mencegah PDF raksasa meledak sebelum ukuran asli terbaca
    if (!containerSize.width || !containerSize.height || !originalPageSize) return 0.1; 
    ...
    return Math.min(scaleX, scaleY) - 0.02; // Kurangi 2% sebagai margin aman
  };

  // --- FUNGSI UPLOAD & RENDER ---
  const handleFileUpload = (event) => {
    ...
    if (file && file.type === 'application/pdf') {
      ...
      setFlipY(false);
      setActiveTool(null);
      setIsZoomMode(false); // Reset mode zoom
    } else {
  ...
  const onPageLoadSuccess = (page) => {
    // Membaca ukuran asli untuk react-pdf versi modern
    setOriginalPageSize({ width: page.width, height: page.height });
  };
  ...
  const handleMouseDown = (e) => {
    if (activeTool !== null) return; // Hanya aktif jika tidak ada tool edit
    setIsDragging(true);
  ...
  // Menentukan Bentuk Kursor Mouse
        ...
        >
          {pdfFile ? (
            <div className="custom-pdf-viewer" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', height: '100%', padding: '20px' }}>
              ...
              {/* Wadah Scroll dengan Mouse Event (Drag/Pan) */}
              <div 
              ...
              >
                <div className="pdf-paper" style={{ boxShadow: '0 10px 30px rgba(0,0,0,0.15)', backgroundColor: 'white' }}>
                  <Document file={pdfFile} onLoadSuccess={onDocumentLoadSuccess} loading={<div style={{ padding: '50px' }}>Memuat mesin...</div>}>
                    <Page 
                      ...
                      style={{
                        ...
                        transition: 'transform 0.1s ease' // Dipercepat agar zoom scroll terasa responsif
                      }}
                    />
                  </Document>
                </div>
              ...
              {/* Floating Action Bar dengan Toggle Scroll Zoom */}
              <div className="floating-action-bar" style={{ position: 'fixed', bottom: '20px', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(5px)', padding: '10px 20px', borderRadius: '99px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', display: 'flex', gap: '10px', alignItems: 'center', zIndex: 1000, border: '1px solid #e5e7eb' }}>
                
                {/* Tombol Toggle Scroll Zoom */}
                <button 
                  className={`action-btn ${isZoomMode ? 'active' : ''}`} 
                  onClick={() => setIsZoomMode(!isZoomMode)} 
                  title="Gunakan roda mouse untuk Zoom"
                  style={{ minWidth: '140px' }}
                >
                  🔍 {isZoomMode ? 'Scroll Zoom: ON' : 'Scroll Zoom: OFF'}
                </button>
                
                <p style={{ fontSize: '11px', color: '#6b7280', margin: '0 5px', minWidth: '40px', textAlign: 'center' }}>
