...
function App() {
  ...
  // 1. Mengukur area kerja yang tersedia
  useEffect(() => {
  ...
  // 2. Menangkap Scroll Mouse untuk Zoom
  useEffect(() => {
    const container = scrollContainerRef.current;
    if (!container) return;

    const handleWheel = (e) => {
      if (!isZoomMode) return; // Jika toggle mati, fungsi berhenti dan browser akan scroll normal
      
      e.preventDefault(); // Blokir scroll web bawaan
      const zoomStep = 0.1; 
      
      if (e.deltaY < 0) {
        setUserZoom((prev) => Math.min(prev + zoomStep, 4)); // Zoom In (Max 400%)
      } else {
        setUserZoom((prev) => Math.max(prev - zoomStep, 0.25)); // Zoom Out (Min 25%)
      }
    };
  ...
  // 3. Kalkulasi Skala Dasar (Fit-to-Box yang Solid)
  const calculateBaseScale = () => {
    // Gunakan skala sangat kecil (0.1) saat memuat agar PDF raksasa tidak merusak layout
    ...
    // Ambil zoom terkecil, dan kurangi 0.02 (2%) sebagai margin aman
  ...
  const onPageLoadSuccess = (page) => {
    // PERBAIKAN: Gunakan page.width (Bukan page.originalWidth)
    setOriginalPageSize({ 
      width: page.width, 
      height: page.height 
    });
            ...
            <div className="custom-pdf-viewer" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', height: '100%', padding: '20px' }}>
              ...
              <div 
                ...
                style={{ 
                  ...
                  width: '100%', 
                  paddingTop: '10px', 
                  paddingBottom: '80px', // Ditambah agar PDF tidak tertutup Action Bar
                ...
                <div className="pdf-paper" style={{ boxShadow: '0 10px 30px rgba(0,0,0,0.15)', backgroundColor: 'white' }}>
                  ...
                  >
                    <Page 
                      ...
                      // PERBAIKAN PENTING: Menggabungkan Auto Zoom dengan User Zoom
                      scale={calculateBaseScale() * userZoom} 
                      
                      // PERBAIKAN PENTING: Menyambungkan fitur Rotasi dan Flip
                      rotate={rotation}
                      style={{
              ...
              <div className="floating-action-bar" style={{ position: 'fixed', bottom: '20px', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(5px)', padding: '10px 20px', borderRadius: '99px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', display: 'flex', gap: '10px', alignItems: 'center', zIndex: 1000, border: '1px solid #e5e7eb' }}>
                ...
                <p style={{ fontSize: '11px', color: '#6b7280', margin: '0 5px', minWidth: '35px', textAlign: 'center' }}>
