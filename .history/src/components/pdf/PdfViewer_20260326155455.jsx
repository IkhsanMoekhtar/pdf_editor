...
export default function PdfViewer({ file, activeTool, setActiveTool, drawings, setDrawings }) {
  // ... (SEMUA STATE DAN LOGIKA SAMA PERSIS SEPERTI SEBELUMNYA) ...
  ...
  useEffect(() => {
    setPageNumber(1); setOriginalPageSize(null); setUserZoom(1); setRotation(0);
  }, [file]);
  ...
  const calculateBaseScale = () => {
    ...
    return Math.min(containerSize.width / originalPageSize.width, containerSize.height / originalPageSize.height) - 0.02;
  };
  ...
  const handleDrawStart = (e) => {
    if (activeTool !== 'draw') return; e.preventDefault();
    const rect = e.currentTarget.getBoundingClientRect();
    setCurrentDrawing([{ x: ((e.clientX - rect.left) / rect.width) * originalPageSize.width, y: ((e.clientY - rect.top) / rect.height) * originalPageSize.height }]);
  };
  const handleDrawMove = (e) => {
    if (activeTool !== 'draw' || !currentDrawing) return; e.preventDefault();
    const rect = e.currentTarget.getBoundingClientRect();
    setCurrentDrawing([...currentDrawing, { x: ((e.clientX - rect.left) / rect.width) * originalPageSize.width, y: ((e.clientY - rect.top) / rect.height) * originalPageSize.height }]);
  };
  const handleDrawEnd = () => {
  ...
  const currentWidth = (originalPageSize ? (originalPageSize.width * calculateBaseScale()) : 0) * userZoom;
      ...
      <div className="custom-pdf-viewer" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', height: '100%', padding: '20px', position:'relative' }}>
        ...
        <div className="pdf-paper-wrapper" ref={scrollContainerRef} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUpOrLeave} onMouseLeave={handleMouseUpOrLeave} style={{ flex: 1, overflow: 'auto', justifyContent: 'center', alignItems: 'flex-start', width: '100%', paddingTop: '10px', paddingBottom: '20px', cursor: getCursorStyle(), userSelect: isDragging ? 'none' : 'auto' }}>
          <div className="pdf-paper" style={{ position: 'relative', boxShadow: '0 10px 30px rgba(0,0,0,0.15)', backgroundColor: 'white', display: 'block', margin: currentWidth > containerSize.width ? '0' : '0 auto', width: currentWidth ? `${currentWidth}px` : 'auto' }}>
            ...
            {originalPageSize && (
              <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: activeTool === 'draw' ? 'auto' : 'none', cursor: activeTool === 'draw' ? 'crosshair' : 'default', zIndex: 10 }} viewBox={`0 0 ${originalPageSize.width} ${originalPageSize.height}`} onPointerDown={handleDrawStart} onPointerMove={handleDrawMove} onPointerUp={handleDrawEnd} onPointerLeave={handleDrawEnd}>
        ...
        <div className="floating-action-bar" style={{ position: 'absolute', bottom: '30px', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(10px)', padding: '12px 24px', borderRadius: '100px', boxShadow: '0 10px 40px rgba(0,0,0,0.12)', display: 'flex', gap: '12px', alignItems: 'center', zIndex: 1000, border: '1px solid rgba(229, 231, 235, 0.8)' }}>
          ...
          <button className={`action-btn ${isZoomMode ? 'active' : ''}`} onClick={() => setIsZoomMode(!isZoomMode)} title="Gunakan Scroll Mouse untuk Zoom" style={{ gap: '6px' }}>
            <ZoomIn size={16} /> <span style={{display: 'none'}}>{/* Opsional: Sembunyikan teks di layar kecil */}</span>
          </button>
