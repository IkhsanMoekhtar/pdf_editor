...
export default function PdfViewer({ file, activeTool, setActiveTool, drawings, setDrawings }) {
    ...
    >
      <div className="custom-pdf-viewer" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', height: '100%', padding: '20px', position:'relative' }}>
        ...
        <div 
          ...
          style={{ 
            flex: 1, overflow: 'auto', justifyContent: 'center', alignItems: 'flex-start', 
            width: '100%', paddingTop: '10px', paddingBottom: '20px',
            cursor: getCursorStyle(), userSelect: isDragging ? 'none' : 'auto' 
          }}
        >
          <div className="pdf-paper" 
            ...
            <Document file={file} onLoadSuccess={onDocumentLoadSuccess} loading={<div style={{ padding: '50px' }}>Memuat mesin dan dokumen...</div>}>
            ...
            {originalPageSize && (
              <svg
                style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: activeTool === 'draw' ? 'auto' : 'none', cursor: activeTool === 'draw' ? 'crosshair' : 'default', zIndex: 10 }}
        ...
        <div className="floating-action-bar" style={{ position: 'absolute', bottom: '20px', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(255,255,255,0.9)', backdropFilter: 'blur(5px)', padding: '10px 20px', borderRadius: '99px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', display: 'flex', gap: '10px', alignItems: 'center', zIndex: 1000, border: '1px solid #e5e7eb' }}>
          ...
          <button className={`action-btn ${isZoomMode ? 'active' : ''}`} onClick={() => setIsZoomMode(!isZoomMode)} title="Gunakan Scroll Mouse untuk Zoom">
            🔍 {isZoomMode ? 'Scroll Zoom: ON' : 'Scroll Zoom: OFF'}
          </button>
          <div style={{ width: '1px', height: '20px', backgroundColor: '#e5e7eb' }}></div>
          <button className={`action-btn ${rotation !== 0 ? 'active' : ''}`} onClick={rotateLeft} title="Putar Kiri">🔄 L</button>
          <button className={`action-btn ${rotation !== 0 ? 'active' : ''}`} onClick={rotateRight} title="Putar Kanan">🔄 R</button>
          <div style={{ width: '1px', height: '20px', backgroundColor: '#e5e7eb' }}></div>
          
          <button className={`action-btn ${activeTool === 'text' ? 'active' : ''}`} onClick={() => setActiveTool(prev => prev === 'text' ? null : 'text')}>✍️ Teks</button>
          <button className={`action-btn ${activeTool === 'draw' ? 'active' : ''}`} onClick={() => setActiveTool(prev => prev === 'draw' ? null : 'draw')}>🖌️ Coret</button>
          ...
          {activeTool === 'draw' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginLeft: '5px', paddingLeft: '10px', borderLeft: '1px solid #e5e7eb' }}>
              ...
              <button className="action-btn" onClick={handleUndo} disabled={drawings.length === 0} style={{ opacity: drawings.length === 0 ? 0.4 : 1, cursor: drawings.length === 0 ? 'not-allowed' : 'pointer' }} title="Undo (Batal)">↩️</button>
              <button className="action-btn" onClick={handleRedo} disabled={redoStack.length === 0} style={{ opacity: redoStack.length === 0 ? 0.4 : 1, cursor: redoStack.length === 0 ? 'not-allowed' : 'pointer' }} title="Redo (Ulangi)">↪️</button>
            </div>
          )}
        </div>
      </div>
