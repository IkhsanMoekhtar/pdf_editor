...
function App() {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      // Wajib: Mencegah browser melakukan scroll naik-turun secara normal
      e.preventDefault(); 
      ...
      if (e.deltaY < 0) {
        setUserZoom((prev) => Math.min(prev + zoomStep, 4)); // Max zoom 400%
      } else {
        setUserZoom((prev) => Math.max(prev - zoomStep, 0.25)); // Min zoom 25%
      }
    };

    // Menambahkan event listener dengan parameter { passive: false } 
    // agar e.preventDefault() di atas diizinkan bekerja oleh browser
    container.addEventListener('wheel', handleWheel, { passive: false });

    // Membersihkan event saat komponen ditutup agar memori tidak bocor
    return () => {
