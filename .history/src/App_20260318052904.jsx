...
function App() {
  ...
  useEffect(() => {
    // Membuat sensor ResizeObserver
    const observer = new ResizeObserver((entries) => {
      if (entries[0]) {
        // Kurangi sedikit (misal 40px) agar ada ruang kosong (margin) di kiri-kanan
        setPdfWidth(entries[0].contentRect.width - 40); 
      }
    });

    // Pasang sensornya ke elemen pembungkus PDF kita
    if (pdfWrapperRef.current) {
      observer.observe(pdfWrapperRef.current);
    }

    // Bersihkan sensor jika komponen ditutup
    return () => observer.disconnect();
  }, [pdfFile]); // Efek ini juga berjalan ulang saat file PDF baru diunggah
