...
function App() {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      // PENTING: Jika mode zoom mati, hentikan fungsi ini. 
      // Browser akan melakukan scroll normal (naik/turun).
      if (!isZoomMode) return; 

      // Jika mode zoom aktif, blokir scroll browser dan lakukan zoom
      e.preventDefault(); 

      const zoomStep = 0.1; 
      if (e.deltaY < 0) {
        setUserZoom((prev) => Math.min(prev + zoomStep, 4)); 
      } else {
        setUserZoom((prev) => Math.max(prev - zoomStep, 0.25)); 
      }
    };
  ...
  const calculateBaseScale = () => {
    ...
    if (!containerSize.width || !containerSize.height || !originalPageSize) return 0.5; 
  ...
  const onPageLoadSuccess = (page) => setOriginalPageSize({ width: page.width, height: page.height });
