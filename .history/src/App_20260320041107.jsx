...
function App() {
  ...
  const handleDrawEnd = () => {
    if (currentDrawing) {
      setDrawings([...drawings, { page: pageNumber, path: currentDrawing }]);
  ...
  const savePdfWithDrawings = async () => {
    if (!pdfFile) return;
    alert("Mempersiapkan dan menyimpan PDF... Mohon tunggu.");
    ...
    try {
        ...
        if (pageIndex >= 0 && pageIndex < pages.length) {
          ...
          for (let i = 0; i < points.length - 1; i++) {
            ...
            page.drawLine({
              ...
              thickness: 3,
              color: rgb(1, 0, 0), // Warna merah (format RGB 0-1)
            });
          }
        }
      });
      ...
      link.download = 'dokumen_teredit.pdf';
      link.click();
              ...
              >
                ...
                {/* --- SVG OVERLAY UNTUK CORETAN --- */}
                  {originalPageSize && (
                    ...
                    >
                      {/* Render coretan yang sudah selesai khusus halaman ini */}
                      ...
                      {/* Render coretan yang sedang ditarik secara realtime */}
                      {currentDrawing && (
                        <path
