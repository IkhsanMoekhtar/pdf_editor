...
function App() {
  ...
  useEffect(() => {
    const handleWheelZoom = (e) => {
      if (e.ctrlKey || e.metaKey) {
        ...
        // --- LOGIKA ZOOM-TO-CURSOR ---
        
        // 1. Dapatkan posisi kursor relatif terhadap container (viewport)
        const rect = container.getBoundingClientRect();
        const mouseX = e.clientX - rect.left;
        const mouseY = e.clientY - rect.top;

        // 2. Hitung posisi titik yang ditunjuk kursor di dalam konten PDF (termasuk scroll)
        ...
        let newZoom;
        if (e.deltaY < 0) {
          // Lepas limiter ke angka besar (misal 20)
          newZoom = Math.min(userZoom + zoomStep, 20);
        } else {
          newZoom = Math.max(userZoom - zoomStep, 0.25);
        }
        ...
        if (newZoom !== userZoom) {
          ...
          // 6. Sesuaikan posisi scroll agar titik konten tetap berada di bawah kursor
          // Gunakan requestAnimationFrame agar eksekusi tepat setelah render ulang
  ...
  }, [userZoom]); // Penting: userZoom harus masuk dependency
