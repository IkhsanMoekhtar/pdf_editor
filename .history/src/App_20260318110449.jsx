...
function App() {
  ...
  // 2. useEffect BARU (Tambahkan di sini)
  useEffect(() => {
    ...
    const handleWheelZoom = (e) => {
      if (isZoomMode) {
        ...
        // Ubah visualZoom secara instan agar UI responsif
        setVisualZoom((prevVisual) => {
          const zoomStep = 0.05; 
          let newZoom = e.deltaY < 0 ? prevVisual + zoomStep : prevVisual - zoomStep;
          newZoom = Math.max(0.25, Math.min(newZoom, 4)); // Batasi batas zoom
          
          // Mulai mode ilusi CSS
          setIsZooming(true);

          // Bersihkan timer sebelumnya jika user masih terus melakukan scroll
          if (zoomTimeoutRef.current) clearTimeout(zoomTimeoutRef.current);
          
          // Buat timer baru: Jika user berhenti scroll selama 300 milidetik, 
          // perbarui 'userZoom' (Ini akan memicu react-pdf merender kualitas HD)
          zoomTimeoutRef.current = setTimeout(() => {
            setUserZoom(newZoom);
            setIsZooming(false); // Matikan mode ilusi CSS
          }, 300);
            ...
            <div className="custom-pdf-viewer" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', height: '100%', padding: '20px' }}>
              ...
              >
                ...
                style={{ 
                  ...
                  transform: `scale(${isZooming ? visualZoom / userZoom : 1})`,
                ...
                >
                  <Page 
                    ...
                    scale={calculateOptimalScale() * userZoom}
                    rotate={rotation} 
                  />
                </Document>
                </div>
              ...
              <div className="floating-action-bar" style={{ position: 'fixed', bottom: '20px', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(255,255,255,0.9)', backdropFilter: 'blur(5px)', padding: '10px 20px', borderRadius: '99px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', display: 'flex', gap: '10px', alignItems: 'center', zIndex: 1000, border: '1px solid #e5e7eb' }}>
                <p style={{ fontSize: '11px', color: '#6b7280', marginRight: '5px' }}>Zoom: {(calculateBaseScale() * visualZoom* 100).toFixed(0)}%</p>
                <button 
