...
useEffect(() => {
  ...
  const handleWheelZoom = (e) => {
      if (isZoomMode) {
        ...
        // 1. Tangkap posisi mouse relatif terhadap container saat ini
        const rect = container.getBoundingClientRect();
        const mouseX = e.clientX - rect.left;
        const mouseY = e.clientY - rect.top;

        // 2. Hitung posisi mouse relatif terhadap konten PDF yang di-zoom
        const scrollX = container.scrollLeft;
        const scrollY = container.scrollTop;
        const contentX = (scrollX + mouseX) / userZoom;
        const contentY = (scrollY + mouseY) / userZoom;

        // 3. Tentukan zoom baru
        let nextZoom;
        if (e.deltaY < 0) {
          nextZoom = Math.min(userZoom + zoomStep, 20);
        } else {
        ...
        // 5. Sesuaikan scroll secara instan agar titik konten tetap di bawah kursor
        // Kita hitung di mana posisi mouse seharusnya berada pada zoom yang baru
  ...
  },[isZoomMode, userZoom]); // Tambahkan userZoom ke dependency agar perhitungan posisi akurat
